...
export class CustomTextboxService {
  /**
  ...
  onInput(eventTarget: EventTarget | null) {
    const value = (eventTarget as HTMLInputElement).value.toString();
    return value;
  }
   ...
   */
  handlePaste(event: ClipboardEvent, isMultiLine: boolean) {
    if (!isMultiLine) {
   ...
   * @param isMultiLine Boolean value of whether the control is multi line.
   */
  handleEnter(event: Event, isMultiLine: boolean) {
    if (!isMultiLine) {
      event.preventDefault();
    }
  }
   ...
   */
  getDefaultValue(fieldDataType: FieldType | undefined, defaultValue: FieldValue | undefined, isMultiLine: boolean) {
